
cardTree = {
	'Warlock': {
		'Minion': {
			'Legendary': {
				6: [{
					'Set': 'TGT',
					'Name': 'Wilfred Fizzlebang'
				}],
				9: [{
					'Set': 'EXPERT1',
					'Name': 'Lord Jaraxxus'
				}, {
					'Set': 'GVG',
					'Name': "Mal'Ganis"
				}]
			},
			'Epic': {
				4: [{
					'Set': 'TGT',
					'Name': 'Dreadsteed'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Pit Lord'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Anima Golem'
				}]
			},
			'Rare': {
				1: [{
					'Set': 'LOE',
					'Name': 'Reliquary Seeker'
				}],
				2: [{
					'Set': 'GVG',
					'Name': 'Mistress of Pain'
				}, {
					'Set': 'TGT',
					'Name': 'Tiny Knight of Evil'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Felguard'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Void Terror'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Fel Cannon'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Doomguard'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Void Crusher'
				}]
			},
			'Common': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Blood Imp'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Flame Imp'
				}],
				2: [{
					'Set': 'TGT',
					'Name': 'Wrathguard'
				}, {
					'Set': 'LOE',
					'Name': 'Dark Peddler'
				}],
				3: [{
					'Set': 'BRM',
					'Name': 'Imp Gang Boss'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Summoning Portal'
				}, {
					'Set': 'NAXX',
					'Name': 'Voidcaller'
				}],
				5: [{
					'Set': 'GVG',
					'Name': 'Floating Watcher'
				}],
				7: [{
					'Set': 'TGT',
					'Name': 'Fearsome Doomguard'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Voidwalker'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Succubus'
				}],
				6: [{
					'Set': 'CORE',
					'Name': 'Dread Infernal'
				}]
			}
		},
		'Spell': {
			'Epic': {
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Bane of Doom'
				}, {
					'Set': 'GVG',
					'Name': 'Demonheart'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Dark Bargain'
				}],
				8: [{
					'Set': 'EXPERT1',
					'Name': 'Twisting Nether'
				}]
			},
			'Rare': {
				3: [{
					'Set': 'BRM',
					'Name': 'Demonwrath'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Imp-losion'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Shadowflame'
				}, {
					'Set': 'TGT',
					'Name': 'Fist of Jaraxxus'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Siphon Soul'
				}]
			},
			'Common': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Power Overwhelming'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Demonfire'
				}, {
					'Set': 'TGT',
					'Name': 'Demonfuse'
				}, {
					'Set': 'LOE',
					'Name': 'Curse of Rafaam'
				}, {
					'Set': 'GVG',
					'Name': 'Darkbomb'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Sense Demons'
				}]
			},
			'Free': {
				0: [{
					'Set': 'CORE',
					'Name': 'Sacrificial Pact'
				}],
				1: [{
					'Set': 'CORE',
					'Name': 'Mortal Coil'
				}, {
					'Set': 'CORE',
					'Name': 'Soulfire'
				}, {
					'Set': 'CORE',
					'Name': 'Corruption'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Drain Life'
				}, {
					'Set': 'CORE',
					'Name': 'Shadow Bolt'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Hellfire'
				}]
			}
		}
	},
	'Warrior': {
		'Minion': {
			'Legendary': {
				6: [{
					'Set': 'GVG',
					'Name': 'Iron Juggernaut'
				}],
				8: [{
					'Set': 'EXPERT1',
					'Name': 'Grommash Hellscream'
				}],
				10: [{
					'Set': 'TGT',
					'Name': 'Varian Wrynn'
				}]
			},
			'Epic': {
				4: [{
					'Set': 'TGT',
					'Name': 'Magnataur Alpha'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Sea Reaver'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Armorsmith'
				}, {
					'Set': 'TGT',
					'Name': "Alexstrasza's Champion"
				}, {
					'Set': 'TGT',
					'Name': 'Sparring Partner'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Frothing Berserker'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Screwjank Clunker'
				}],
				5: [{
					'Set': 'GVG',
					'Name': 'Siege Engine'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Shieldmaiden'
				}]
			},
			'Common': {
				1: [{
					'Set': 'GVG',
					'Name': 'Warbot'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Cruel Taskmaster'
				}],
				3: [{
					'Set': 'LOE',
					'Name': 'Fierce Monkey'
				}, {
					'Set': 'TGT',
					'Name': 'Orgrimmar Aspirant'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Arathi Weaponsmith'
				}, {
					'Set': 'BRM',
					'Name': 'Axe Flinger'
				}],
				7: [{
					'Set': 'LOE',
					'Name': 'Obsidian Destroyer'
				}]
			},
			'Free': {
				3: [{
					'Set': 'CORE',
					'Name': 'Warsong Commander'
				}],
				4: [{
					'Set': 'CORE',
					'Name': "Kor'kron Elite"
				}]
			}
		},
		'Weapon': {
			'Epic': {
				7: [{
					'Set': 'EXPERT1',
					'Name': 'Gorehowl'
				}]
			},
			'Rare': {
				1: [{
					'Set': 'LOE',
					'Name': 'Cursed Blade'
				}],
				3: [{
					'Set': 'TGT',
					'Name': "King's Defender"
				}]
			},
			'Common': {
				3: [{
					'Set': 'GVG',
					'Name': 'Ogre Warmaul'
				}],
				4: [{
					'Set': 'NAXX',
					'Name': "Death's Bite"
				}]
			},
			'Free': {
				2: [{
					'Set': 'CORE',
					'Name': 'Fiery War Axe'
				}],
				5: [{
					'Set': 'CORE',
					'Name': 'Arcanite Reaper'
				}]
			}
		},
		'Spell': {
			'Epic': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Shield Slam'
				}],
				3: [{
					'Set': 'GVG',
					'Name': 'Bouncing Blade'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Brawl'
				}],
				7: [{
					'Set': 'GVG',
					'Name': 'Crush'
				}]
			},
			'Rare': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Upgrade!'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Commanding Shout'
				}, {
					'Set': 'BRM',
					'Name': 'Revenge'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Mortal Strike'
				}]
			},
			'Common': {
				0: [{
					'Set': 'EXPERT1',
					'Name': 'Inner Rage'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Rampage'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Slam'
				}, {
					'Set': 'TGT',
					'Name': 'Bolster'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Battle Rage'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Bash'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Execute'
				}, {
					'Set': 'CORE',
					'Name': 'Whirlwind'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Heroic Strike'
				}, {
					'Set': 'CORE',
					'Name': 'Cleave'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Shield Block'
				}, {
					'Set': 'CORE',
					'Name': 'Charge'
				}]
			}
		}
	},
	'Paladin': {
		'Minion': {
			'Legendary': {
				5: [{
					'Set': 'GVG',
					'Name': 'Bolvar Fordragon'
				}],
				7: [{
					'Set': 'TGT',
					'Name': 'Eadric the Pure'
				}],
				8: [{
					'Set': 'EXPERT1',
					'Name': 'Tirion Fordring'
				}]
			},
			'Epic': {
				5: [{
					'Set': 'GVG',
					'Name': 'Quartermaster'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Mysterious Challenger'
				}]
			},
			'Rare': {
				3: [{
					'Set': 'GVG',
					'Name': 'Scarlet Purifier'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Aldor Peacekeeper'
				}],
				5: [{
					'Set': 'BRM',
					'Name': 'Dragon Consort'
				}, {
					'Set': 'GVG',
					'Name': 'Cobalt Guardian'
				}, {
					'Set': 'TGT',
					'Name': 'Tuskarr Jouster'
				}]
			},
			'Common': {
				2: [{
					'Set': 'GVG',
					'Name': 'Shielded Minibot'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Argent Protector'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Warhorse Trainer'
				}],
				4: [{
					'Set': 'LOE',
					'Name': 'Keeper of Uldaman'
				}, {
					'Set': 'TGT',
					'Name': 'Murloc Knight'
				}]
			},
			'Free': {
				7: [{
					'Set': 'CORE',
					'Name': 'Guardian of Kings'
				}]
			}
		},
		'Weapon': {
			'Epic': {
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Sword of Justice'
				}, {
					'Set': 'GVG',
					'Name': 'Coghammer'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'TGT',
					'Name': 'Argent Lance'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': "Light's Justice"
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Truesilver Champion'
				}]
			}
		},
		'Spell': {
			'Epic': {
				6: [{
					'Set': 'TGT',
					'Name': 'Enter the Coliseum'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Avenging Wrath'
				}],
				8: [{
					'Set': 'EXPERT1',
					'Name': 'Lay on Hands'
				}]
			},
			'Rare': {
				1: [{
					'Set': 'TGT',
					'Name': 'Competitive Spirit'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Equality'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Divine Favor'
				}, {
					'Set': 'GVG',
					'Name': 'Muster for Battle'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Holy Wrath'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Blessed Champion'
				}],
				10: [{
					'Set': 'LOE',
					'Name': 'Anyfin Can Happen'
				}]
			},
			'Common': {
				1: [{
					'Set': 'NAXX',
					'Name': 'Avenge'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Redemption'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Eye for an Eye'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Blessing of Wisdom'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Noble Sacrifice'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Repentance'
				}, {
					'Set': 'LOE',
					'Name': 'Sacred Trial'
				}],
				2: [{
					'Set': 'GVG',
					'Name': 'Seal of Light'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Seal of Champions'
				}],
				5: [{
					'Set': 'BRM',
					'Name': 'Solemn Vigil'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Blessing of Might'
				}, {
					'Set': 'CORE',
					'Name': 'Humility'
				}, {
					'Set': 'CORE',
					'Name': 'Hand of Protection'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Holy Light'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Hammer of Wrath'
				}, {
					'Set': 'CORE',
					'Name': 'Consecration'
				}, {
					'Set': 'CORE',
					'Name': 'Blessing of Kings'
				}]
			}
		}
	},
	'Hunter': {
		'Minion': {
			'Legendary': {
				3: [{
					'Set': 'TGT',
					'Name': 'Dreadscale'
				}],
				7: [{
					'Set': 'TGT',
					'Name': 'Acidmaw'
				}, {
					'Set': 'GVG',
					'Name': "Gahz'rilla"
				}],
				9: [{
					'Set': 'EXPERT1',
					'Name': 'King Krush'
				}]
			},
			'Epic': {
				2: [{
					'Set': 'GVG',
					'Name': 'Steamwheedle Sniper'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Stablemaster'
				}]
			},
			'Rare': {
				3: [{
					'Set': 'GVG',
					'Name': 'Metaltooth Leaper'
				}],
				4: [{
					'Set': 'BRM',
					'Name': 'Core Rager'
				}],
				5: [{
					'Set': 'TGT',
					'Name': 'Ram Wrangler'
				}, {
					'Set': 'GVG',
					'Name': 'King of Beasts'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Savannah Highmane'
				}]
			},
			'Common': {
				1: [{
					'Set': 'TGT',
					'Name': 'Brave Archer'
				}, {
					'Set': 'NAXX',
					'Name': 'Webspinner'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Scavenging Hyena'
				}, {
					'Set': 'TGT',
					'Name': "King's Elekk"
				}],
				3: [{
					'Set': 'LOE',
					'Name': 'Desert Camel'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Timber Wolf'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Houndmaster'
				}],
				5: [{
					'Set': 'CORE',
					'Name': 'Starving Buzzard'
				}, {
					'Set': 'CORE',
					'Name': 'Tundra Rhino'
				}]
			}
		},
		'Weapon': {
			'Epic': {
				7: [{
					'Set': 'EXPERT1',
					'Name': "Gladiator's Longbow"
				}]
			},
			'Rare': {
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Eaglehorn Bow'
				}]
			},
			'Common': {
				2: [{
					'Set': 'GVG',
					'Name': 'Glaivezooka'
				}]
			}
		},
		'Spell': {
			'Epic': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Bestial Wrath'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Snake Trap'
				}, {
					'Set': 'GVG',
					'Name': 'Feign Death'
				}, {
					'Set': 'TGT',
					'Name': 'Lock and Load'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Misdirection'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Flare'
				}, {
					'Set': 'LOE',
					'Name': "Explorer's Hat"
				}, {
					'Set': 'GVG',
					'Name': 'Call Pet'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Powershot'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Explosive Shot'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Ball of Spiders'
				}]
			},
			'Common': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Snipe'
				}, {
					'Set': 'BRM',
					'Name': 'Quick Shot'
				}, {
					'Set': 'LOE',
					'Name': 'Dart Trap'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Explosive Trap'
				}, {
					'Set': 'TGT',
					'Name': 'Bear Trap'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Freezing Trap'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Deadly Shot'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Unleash the Hounds'
				}],
				5: [{
					'Set': 'GVG',
					'Name': 'Cobra Shot'
				}]
			},
			'Free': {
				0: [{
					'Set': 'CORE',
					'Name': "Hunter's Mark"
				}],
				1: [{
					'Set': 'CORE',
					'Name': 'Tracking'
				}, {
					'Set': 'CORE',
					'Name': 'Arcane Shot'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Animal Companion'
				}, {
					'Set': 'CORE',
					'Name': 'Kill Command'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Multi-Shot'
				}]
			}
		}
	},
	'Druid': {
		'Minion': {
			'Legendary': {
				7: [{
					'Set': 'GVG',
					'Name': 'Malorne'
				}],
				9: [{
					'Set': 'TGT',
					'Name': 'Aviana'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Cenarius'
				}]
			},
			'Epic': {
				7: [{
					'Set': 'EXPERT1',
					'Name': 'Ancient of War'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ancient of Lore'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'TGT',
					'Name': 'Darnassus Aspirant'
				}],
				3: [{
					'Set': 'GVG',
					'Name': 'Grove Tender'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Savage Combatant'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Keeper of the Grove'
				}, {
					'Set': 'LOE',
					'Name': 'Jungle Moonkin'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Mech-Bear-Cat'
				}],
				7: [{
					'Set': 'TGT',
					'Name': 'Knight of the Wild'
				}],
				9: [{
					'Set': 'BRM',
					'Name': 'Volcanic Lumberer'
				}]
			},
			'Common': {
				2: [{
					'Set': 'TGT',
					'Name': 'Druid of the Saber'
				}, {
					'Set': 'GVG',
					'Name': 'Anodized Robo Cub'
				}],
				3: [{
					'Set': 'LOE',
					'Name': 'Mounted Raptor'
				}, {
					'Set': 'BRM',
					'Name': 'Druid of the Flame'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Wildwalker'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Druid of the Claw'
				}, {
					'Set': 'GVG',
					'Name': 'Druid of the Fang'
				}]
			},
			'Free': {
				8: [{
					'Set': 'CORE',
					'Name': 'Ironbark Protector'
				}]
			}
		},
		'Spell': {
			'Epic': {
				3: [{
					'Set': 'TGT',
					'Name': 'Mulch'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Astral Communion'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Dark Wispers'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Force of Nature'
				}],
				9: [{
					'Set': 'GVG',
					'Name': 'Tree of Life'
				}]
			},
			'Rare': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Savagery'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Bite'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Starfall'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Nourish'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Recycle'
				}]
			},
			'Common': {
				1: [{
					'Set': 'TGT',
					'Name': 'Living Roots'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Naturalize'
				}, {
					'Set': 'LOE',
					'Name': 'Raven Idol'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Power of the Wild'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Wrath'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Mark of Nature'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Soul of the Forest'
				}, {
					'Set': 'NAXX',
					'Name': 'Poison Seeds'
				}]
			},
			'Free': {
				0: [{
					'Set': 'CORE',
					'Name': 'Innervate'
				}, {
					'Set': 'CORE',
					'Name': 'Moonfire'
				}],
				1: [{
					'Set': 'CORE',
					'Name': 'Claw'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Mark of the Wild'
				}, {
					'Set': 'CORE',
					'Name': 'Wild Growth'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Savage Roar'
				}, {
					'Set': 'CORE',
					'Name': 'Healing Touch'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Swipe'
				}],
				6: [{
					'Set': 'CORE',
					'Name': 'Starfire'
				}]
			}
		}
	},
	'Neutral': {
		'Minion': {
			'Legendary': {
				1: [{
					'Set': 'LOE',
					'Name': 'Sir Finley Mrrgglton'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Nat Pagle'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Bloodmage Thalnos'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Lorewalker Cho'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Millhouse Manastorm'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Eydis Darkbane'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Tinkmaster Overspark'
				}, {
					'Set': 'LOE',
					'Name': 'Brann Bronzebeard'
				}, {
					'Set': 'EXPERT1',
					'Name': 'King Mukla'
				}, {
					'Set': 'TGT',
					'Name': 'Fjola Lightbane'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Gormok the Impaler'
				}, {
					'Set': 'NAXX',
					'Name': 'Baron Rivendare'
				}, {
					'Set': 'LOE',
					'Name': 'Elise Starseeker'
				}, {
					'Set': 'REWARD',
					'Name': 'Old Murk-Eye'
				}],
				5: [{
					'Set': 'NAXX',
					'Name': 'Loatheb'
				}, {
					'Set': 'NAXX',
					'Name': 'Feugen'
				}, {
					'Set': 'TGT',
					'Name': 'Nexus-Champion Saraad'
				}, {
					'Set': 'GVG',
					'Name': "Mimiron's Head"
				}, {
					'Set': 'NAXX',
					'Name': 'Stalagg'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Harrison Jones'
				}, {
					'Set': 'GVG',
					'Name': 'Hemet Nesingwary'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Captain Greenskin'
				}, {
					'Set': 'PROMO',
					'Name': 'Elite Tauren Chieftain'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Leeroy Jenkins'
				}, {
					'Set': 'GVG',
					'Name': 'Blingtron 3000'
				}],
				6: [{
					'Set': 'LOE',
					'Name': 'Reno Jackson'
				}, {
					'Set': 'BRM',
					'Name': 'Emperor Thaurissan'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Hogger'
				}, {
					'Set': 'TGT',
					'Name': 'The Skeleton Knight'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Illidan Stormrage'
				}, {
					'Set': 'NAXX',
					'Name': 'Maexxna'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Cairne Bloodhoof'
				}, {
					'Set': 'GVG',
					'Name': 'Toshley'
				}, {
					'Set': 'EXPERT1',
					'Name': 'The Beast'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Sylvanas Windrunner'
				}, {
					'Set': 'GVG',
					'Name': 'Mogor the Ogre'
				}, {
					'Set': 'TGT',
					'Name': 'Justicar Trueheart'
				}, {
					'Set': 'TGT',
					'Name': 'Bolf Ramshield'
				}, {
					'Set': 'GVG',
					'Name': 'Gazlowe'
				}, {
					'Set': 'EXPERT1',
					'Name': 'The Black Knight'
				}, {
					'Set': 'PROMO',
					'Name': 'Gelbin Mekkatorque'
				}],
				7: [{
					'Set': 'GVG',
					'Name': 'Troggzor the Earthinator'
				}, {
					'Set': 'BRM',
					'Name': 'Rend Blackhand'
				}, {
					'Set': 'TGT',
					'Name': 'Chillmaw'
				}, {
					'Set': 'TGT',
					'Name': "Skycap'n Kragg"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Baron Geddon'
				}, {
					'Set': 'GVG',
					'Name': 'Dr. Boom'
				}],
				8: [{
					'Set': 'BRM',
					'Name': 'Chromaggus'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Gruul'
				}, {
					'Set': 'NAXX',
					'Name': "Kel'Thuzad"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ragnaros the Firelord'
				}, {
					'Set': 'GVG',
					'Name': "Sneed's Old Shredder"
				}, {
					'Set': 'GVG',
					'Name': 'Foe Reaper 4000'
				}],
				9: [{
					'Set': 'BRM',
					'Name': 'Nefarian'
				}, {
					'Set': 'BRM',
					'Name': 'Majordomo Executus'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ysera'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Nozdormu'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Alexstrasza'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Malygos'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Onyxia'
				}, {
					'Set': 'TGT',
					'Name': 'Icehowl'
				}, {
					'Set': 'LOE',
					'Name': 'Arch-Thief Rafaam'
				}, {
					'Set': 'GVG',
					'Name': 'Mekgineer Thermaplugg'
				}],
				10: [{
					'Set': 'EXPERT1',
					'Name': 'Deathwing'
				}]
			},
			'Epic': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Hungry Crab'
				}],
				2: [{
					'Set': 'TGT',
					'Name': 'Garrison Commander'
				}, {
					'Set': 'NAXX',
					'Name': 'Echoing Ooze'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Doomsayer'
				}, {
					'Set': 'REWARD',
					'Name': "Captain's Parrot"
				}, {
					'Set': 'GVG',
					'Name': 'Recombobulator'
				}],
				3: [{
					'Set': 'GVG',
					'Name': 'Hobgoblin'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Big Game Hunter'
				}, {
					'Set': 'NAXX',
					'Name': 'Shade of Naxxramas'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Murloc Warleader'
				}, {
					'Set': 'TGT',
					'Name': 'Master of Ceremonies'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Southsea Captain'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Blood Knight'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Mini-Mage'
				}, {
					'Set': 'TGT',
					'Name': 'Crowd Favorite'
				}, {
					'Set': 'TGT',
					'Name': 'Twilight Guardian'
				}, {
					'Set': 'GVG',
					'Name': 'Enhance-o Mechano'
				}],
				5: [{
					'Set': 'GVG',
					'Name': 'Fel Reaver'
				}, {
					'Set': 'LOE',
					'Name': 'Djinni of Zephyrs'
				}, {
					'Set': 'LOE',
					'Name': 'Naga Sea Witch'
				}, {
					'Set': 'TGT',
					'Name': 'Recruiter'
				}, {
					'Set': 'GVG',
					'Name': 'Junkbot'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Faceless Manipulator'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Sideshow Spelleater'
				}, {
					'Set': 'TGT',
					'Name': 'Grand Crusader'
				}, {
					'Set': 'GVG',
					'Name': 'Piloted Sky Golem'
				}, {
					'Set': 'TGT',
					'Name': 'Kodorider'
				}],
				10: [{
					'Set': 'EXPERT1',
					'Name': 'Sea Giant'
				}, {
					'Set': 'TGT',
					'Name': 'Frost Giant'
				}],
				12: [{
					'Set': 'EXPERT1',
					'Name': 'Mountain Giant'
				}, {
					'Set': 'GVG',
					'Name': 'Clockwork Giant'
				}],
				20: [{
					'Set': 'EXPERT1',
					'Name': 'Molten Giant'
				}]
			},
			'Rare': {
				0: [{
					'Set': 'GVG',
					'Name': 'Target Dummy'
				}],
				1: [{
					'Set': 'BRM',
					'Name': 'Dragon Egg'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Murloc Tidecaller'
				}, {
					'Set': 'TGT',
					'Name': 'Injured Kvaldir'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Angry Chicken'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Secretkeeper'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Young Priestess'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Bloodsail Corsair'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Lightwarden'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Master Swordsmith'
				}, {
					'Set': 'TGT',
					'Name': 'Argent Watchman'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Wild Pyromancer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Pint-Sized Summoner'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Crazed Alchemist'
				}, {
					'Set': 'NAXX',
					'Name': 'Nerubian Egg'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Mana Wraith'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Sunfury Protector'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ancient Watcher'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Knife Juggler'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Mana Addict'
				}],
				3: [{
					'Set': 'GVG',
					'Name': 'Illuminator'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Questing Adventurer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Alarm-o-Bot'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Injured Blademaster'
				}, {
					'Set': 'TGT',
					'Name': "Light's Champion"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Demolisher'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Mind Control Tech'
				}, {
					'Set': 'TGT',
					'Name': 'Coliseum Manager'
				}, {
					'Set': 'TGT',
					'Name': 'Fencing Coach'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Emperor Cobra'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Coldlight Oracle'
				}, {
					'Set': 'GVG',
					'Name': "Lil' Exorcist"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Arcane Golem'
				}, {
					'Set': 'TGT',
					'Name': 'Saboteur'
				}, {
					'Set': 'NAXX',
					'Name': 'Deathlord'
				}, {
					'Set': 'GVG',
					'Name': 'Goblin Sapper'
				}, {
					'Set': 'GVG',
					'Name': 'Gnomish Experimenter'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Coldlight Seer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Imp Master'
				}],
				4: [{
					'Set': 'LOE',
					'Name': 'Eerie Statue'
				}, {
					'Set': 'TGT',
					'Name': 'Armored Warhorse'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ancient Mage'
				}, {
					'Set': 'GVG',
					'Name': 'Kezan Mystic'
				}, {
					'Set': 'NAXX',
					'Name': 'Wailing Soul'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Twilight Drake'
				}, {
					'Set': 'GVG',
					'Name': 'Arcane Nullifier X-21'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Defender of Argus'
				}, {
					'Set': 'GVG',
					'Name': 'Jeeves'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Violet Teacher'
				}, {
					'Set': 'LOE',
					'Name': 'Ancient Shade'
				}],
				5: [{
					'Set': 'BRM',
					'Name': 'Grim Patron'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Stampeding Kodo'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Azure Drake'
				}, {
					'Set': 'NAXX',
					'Name': 'Sludge Belcher'
				}, {
					'Set': 'GVG',
					'Name': 'Bomb Lobber'
				}, {
					'Set': 'LOE',
					'Name': 'Summoning Stone'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Abomination'
				}, {
					'Set': 'GVG',
					'Name': 'Madder Bomber'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Master Jouster'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Sunwalker'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Gadgetzan Auctioneer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Argent Commander'
				}, {
					'Set': 'LOE',
					'Name': 'Wobbling Runts'
				}, {
					'Set': 'TGT',
					'Name': "Mogor's Champion"
				}],
				7: [{
					'Set': 'EXPERT1',
					'Name': 'Ravenholdt Assassin'
				}]
			},
			'Common': {
				0: [{
					'Set': 'LOE',
					'Name': 'Murloc Tinyfin'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Wisp'
				}],
				1: [{
					'Set': 'NAXX',
					'Name': 'Undertaker'
				}, {
					'Set': 'TGT',
					'Name': 'Tournament Attendee'
				}, {
					'Set': 'GVG',
					'Name': 'Clockwork Gnome'
				}, {
					'Set': 'TGT',
					'Name': 'Gadgetzan Jouster'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Young Dragonhawk'
				}, {
					'Set': 'NAXX',
					'Name': 'Zombie Chow'
				}, {
					'Set': 'TGT',
					'Name': 'Lowly Squire'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Southsea Deckhand'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Abusive Sergeant'
				}, {
					'Set': 'GVG',
					'Name': 'Cogmaster'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Shieldbearer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Leper Gnome'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Worgen Infiltrator'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Argent Squire'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Mad Bomber'
				}, {
					'Set': 'GVG',
					'Name': 'Stonesplinter Trogg'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Youthful Brewmaster'
				}, {
					'Set': 'GVG',
					'Name': 'Mechwarper'
				}, {
					'Set': 'TGT',
					'Name': 'Flame Juggler'
				}, {
					'Set': 'GVG',
					'Name': 'Micro Machine'
				}, {
					'Set': 'GVG',
					'Name': 'Explosive Sheep'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Amani Berserker'
				}, {
					'Set': 'NAXX',
					'Name': 'Haunted Creeper'
				}, {
					'Set': 'LOE',
					'Name': 'Jeweled Scarab'
				}, {
					'Set': 'TGT',
					'Name': 'Lance Carrier'
				}, {
					'Set': 'NAXX',
					'Name': 'Unstable Ghoul'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Bloodsail Raider'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Loot Hoarder'
				}, {
					'Set': 'GVG',
					'Name': "Ship's Cannon"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ironbeak Owl'
				}, {
					'Set': 'GVG',
					'Name': 'Gilblin Stalker'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Dire Wolf Alpha'
				}, {
					'Set': 'NAXX',
					'Name': 'Mad Scientist'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Faerie Dragon'
				}, {
					'Set': 'NAXX',
					'Name': "Nerub'ar Weblord"
				}, {
					'Set': 'LOE',
					'Name': 'Huge Toad'
				}, {
					'Set': 'TGT',
					'Name': 'Boneguard Lieutenant'
				}, {
					'Set': 'GVG',
					'Name': 'Puddlestomper'
				}, {
					'Set': 'GVG',
					'Name': 'Annoy-o-Tron'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Silent Knight'
				}, {
					'Set': 'GVG',
					'Name': 'Gnomeregan Infantry'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Raging Worgen'
				}, {
					'Set': 'BRM',
					'Name': 'Blackwing Technician'
				}, {
					'Set': 'GVG',
					'Name': 'Tinkertown Technician'
				}, {
					'Set': 'TGT',
					'Name': 'Argent Horserider'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Thrallmar Farseer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Harvest Golem'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Jungle Panther'
				}, {
					'Set': 'TGT',
					'Name': 'Ice Rager'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Acolyte of Pain'
				}, {
					'Set': 'GVG',
					'Name': 'Spider Tank'
				}, {
					'Set': 'GVG',
					'Name': 'Flying Machine'
				}, {
					'Set': 'TGT',
					'Name': 'Dragonhawk Rider'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Earthen Ring Farseer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Flesheating Ghoul'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Scarlet Crusader'
				}, {
					'Set': 'TGT',
					'Name': 'Silver Hand Regent'
				}, {
					'Set': 'NAXX',
					'Name': 'Stoneskin Gargoyle'
				}, {
					'Set': 'NAXX',
					'Name': 'Dancing Swords'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Tauren Warrior'
				}, {
					'Set': 'GVG',
					'Name': 'Ogre Brute'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Piloted Shredder'
				}, {
					'Set': 'EXPERT1',
					'Name': "Mogu'shan Warden"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Spellbreaker'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Dark Iron Dwarf'
				}, {
					'Set': 'TGT',
					'Name': 'Refreshment Vendor'
				}, {
					'Set': 'GVG',
					'Name': 'Burly Rockjaw Trogg'
				}, {
					'Set': 'TGT',
					'Name': 'Maiden of the Lake'
				}, {
					'Set': 'TGT',
					'Name': 'Tournament Medic'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Dread Corsair'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ancient Brewmaster'
				}, {
					'Set': 'GVG',
					'Name': 'Lost Tallstrider'
				}, {
					'Set': 'LOE',
					'Name': 'Tomb Spider'
				}, {
					'Set': 'BRM',
					'Name': 'Dragonkin Sorcerer'
				}, {
					'Set': 'LOE',
					'Name': 'Gorillabot A-3'
				}, {
					'Set': 'BRM',
					'Name': 'Hungry Dragon'
				}, {
					'Set': 'TGT',
					'Name': 'Evil Heckler'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Cult Master'
				}, {
					'Set': 'GVG',
					'Name': 'Mechanical Yeti'
				}, {
					'Set': 'TGT',
					'Name': 'Frigid Snobold'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Silvermoon Guardian'
				}],
				5: [{
					'Set': 'LOE',
					'Name': 'Anubisath Sentinel'
				}, {
					'Set': 'TGT',
					'Name': 'Kvaldir Raider'
				}, {
					'Set': 'BRM',
					'Name': 'Blackwing Corruptor'
				}, {
					'Set': 'TGT',
					'Name': "Mukla's Champion"
				}, {
					'Set': 'TGT',
					'Name': 'Clockwork Knight'
				}, {
					'Set': 'GVG',
					'Name': 'Salty Dog'
				}, {
					'Set': 'TGT',
					'Name': 'Pit Fighter'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Venture Co. Mercenary'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Stranglethorn Tiger'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Silver Hand Knight'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Spiteful Smith'
				}, {
					'Set': 'GVG',
					'Name': 'Antique Healbot'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Fen Creeper'
				}, {
					'Set': 'NAXX',
					'Name': 'Spectral Knight'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Windfury Harpy'
				}, {
					'Set': 'BRM',
					'Name': 'Drakonid Crusher'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Priestess of Elune'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Frost Elemental'
				}, {
					'Set': 'BRM',
					'Name': 'Volcanic Drake'
				}],
				7: [{
					'Set': 'TGT',
					'Name': 'Captured Jormungar'
				}],
				8: [{
					'Set': 'GVG',
					'Name': 'Force-Tank MAX'
				}, {
					'Set': 'LOE',
					'Name': 'Fossilized Devilsaur'
				}],
				9: [{
					'Set': 'TGT',
					'Name': 'North Sea Kraken'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Voodoo Doctor'
				}, {
					'Set': 'CORE',
					'Name': 'Elven Archer'
				}, {
					'Set': 'CORE',
					'Name': 'Murloc Raider'
				}, {
					'Set': 'CORE',
					'Name': 'Grimscale Oracle'
				}, {
					'Set': 'CORE',
					'Name': 'Stonetusk Boar'
				}, {
					'Set': 'CORE',
					'Name': 'Goldshire Footman'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'River Crocolisk'
				}, {
					'Set': 'CORE',
					'Name': 'Kobold Geomancer'
				}, {
					'Set': 'CORE',
					'Name': 'Acidic Swamp Ooze'
				}, {
					'Set': 'CORE',
					'Name': 'Bluegill Warrior'
				}, {
					'Set': 'CORE',
					'Name': 'Bloodfen Raptor'
				}, {
					'Set': 'CORE',
					'Name': 'Novice Engineer'
				}, {
					'Set': 'CORE',
					'Name': 'Murloc Tidehunter'
				}, {
					'Set': 'CORE',
					'Name': 'Frostwolf Grunt'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Ironfur Grizzly'
				}, {
					'Set': 'CORE',
					'Name': 'Razorfen Hunter'
				}, {
					'Set': 'CORE',
					'Name': 'Silverback Patriarch'
				}, {
					'Set': 'CORE',
					'Name': 'Magma Rager'
				}, {
					'Set': 'CORE',
					'Name': 'Dalaran Mage'
				}, {
					'Set': 'CORE',
					'Name': 'Ironforge Rifleman'
				}, {
					'Set': 'CORE',
					'Name': 'Wolfrider'
				}, {
					'Set': 'CORE',
					'Name': 'Shattered Sun Cleric'
				}, {
					'Set': 'CORE',
					'Name': 'Raid Leader'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Dragonling Mechanic'
				}, {
					'Set': 'CORE',
					'Name': 'Chillwind Yeti'
				}, {
					'Set': 'CORE',
					'Name': "Sen'jin Shieldmasta"
				}, {
					'Set': 'CORE',
					'Name': 'Ogre Magi'
				}, {
					'Set': 'CORE',
					'Name': 'Stormwind Knight'
				}, {
					'Set': 'CORE',
					'Name': 'Oasis Snapjaw'
				}, {
					'Set': 'CORE',
					'Name': 'Gnomish Inventor'
				}],
				5: [{
					'Set': 'CORE',
					'Name': 'Darkscale Healer'
				}, {
					'Set': 'CORE',
					'Name': 'Nightblade'
				}, {
					'Set': 'CORE',
					'Name': 'Stormpike Commando'
				}, {
					'Set': 'CORE',
					'Name': 'Frostwolf Warlord'
				}, {
					'Set': 'CORE',
					'Name': 'Booty Bay Bodyguard'
				}, {
					'Set': 'CORE',
					'Name': 'Gurubashi Berserker'
				}],
				6: [{
					'Set': 'CORE',
					'Name': 'Reckless Rocketeer'
				}, {
					'Set': 'CORE',
					'Name': 'Archmage'
				}, {
					'Set': 'CORE',
					'Name': 'Lord of the Arena'
				}, {
					'Set': 'CORE',
					'Name': 'Boulderfist Ogre'
				}],
				7: [{
					'Set': 'CORE',
					'Name': 'War Golem'
				}, {
					'Set': 'CORE',
					'Name': 'Core Hound'
				}, {
					'Set': 'CORE',
					'Name': 'Stormwind Champion'
				}]
			}
		}
	},
	'Rogue': {
		'Minion': {
			'Legendary': {
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Edwin VanCleef'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Trade Prince Gallywix'
				}],
				9: [{
					'Set': 'TGT',
					'Name': "Anub'arak"
				}]
			},
			'Epic': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Patient Assassin'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Kidnapper'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'TGT',
					'Name': 'Cutpurse'
				}, {
					'Set': 'GVG',
					'Name': 'One-eyed Cheat'
				}],
				3: [{
					'Set': 'GVG',
					'Name': 'Iron Sensei'
				}, {
					'Set': 'TGT',
					'Name': 'Shady Dealer'
				}, {
					'Set': 'LOE',
					'Name': 'Unearthed Raptor'
				}, {
					'Set': 'EXPERT1',
					'Name': 'SI:7 Agent'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Master of Disguise'
				}],
				5: [{
					'Set': 'GVG',
					'Name': 'Ogre Ninja'
				}, {
					'Set': 'BRM',
					'Name': 'Dark Iron Skulker'
				}]
			},
			'Common': {
				1: [{
					'Set': 'TGT',
					'Name': 'Buccaneer'
				}, {
					'Set': 'LOE',
					'Name': 'Pit Snake'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Defias Ringleader'
				}, {
					'Set': 'GVG',
					'Name': 'Goblin Auto-Barber'
				}, {
					'Set': 'TGT',
					'Name': 'Undercity Valiant'
				}],
				4: [{
					'Set': 'NAXX',
					'Name': "Anub'ar Ambusher"
				}, {
					'Set': 'LOE',
					'Name': 'Tomb Pillager'
				}],
				5: [{
					'Set': 'TGT',
					'Name': 'Shado-Pan Rider'
				}]
			}
		},
		'Weapon': {
			'Epic': {
				3: [{
					'Set': 'GVG',
					'Name': "Cogmaster's Wrench"
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Poisoned Blade'
				}]
			},
			'Rare': {
				3: [{
					'Set': 'EXPERT1',
					'Name': "Perdition's Blade"
				}]
			},
			'Free': {
				5: [{
					'Set': 'CORE',
					'Name': "Assassin's Blade"
				}]
			}
		},
		'Spell': {
			'Epic': {
				0: [{
					'Set': 'EXPERT1',
					'Name': 'Preparation'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Beneath the Grounds'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Sabotage'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Blade Flurry'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Burgle'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Headcrack'
				}]
			},
			'Common': {
				0: [{
					'Set': 'EXPERT1',
					'Name': 'Shadowstep'
				}],
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Conceal'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Cold Blood'
				}],
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Betrayal'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Eviscerate'
				}, {
					'Set': 'BRM',
					'Name': 'Gang Up'
				}],
				4: [{
					'Set': 'GVG',
					'Name': "Tinker's Sharpsword Oil"
				}]
			},
			'Free': {
				0: [{
					'Set': 'CORE',
					'Name': 'Backstab'
				}],
				1: [{
					'Set': 'CORE',
					'Name': 'Sinister Strike'
				}, {
					'Set': 'CORE',
					'Name': 'Deadly Poison'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Shiv'
				}, {
					'Set': 'CORE',
					'Name': 'Sap'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Fan of Knives'
				}],
				5: [{
					'Set': 'CORE',
					'Name': 'Assassinate'
				}],
				6: [{
					'Set': 'CORE',
					'Name': 'Vanish'
				}],
				7: [{
					'Set': 'CORE',
					'Name': 'Sprint'
				}]
			}
		}
	},
	'Mage': {
		'Minion': {
			'Legendary': {
				7: [{
					'Set': 'EXPERT1',
					'Name': 'Archmage Antonidas'
				}, {
					'Set': 'GVG',
					'Name': 'Flame Leviathan'
				}],
				8: [{
					'Set': 'TGT',
					'Name': 'Rhonin'
				}]
			},
			'Epic': {
				4: [{
					'Set': 'GVG',
					'Name': 'Wee Spellstopper'
				}],
				6: [{
					'Set': 'TGT',
					'Name': 'Coldarra Drake'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'TGT',
					'Name': 'Fallen Hero'
				}],
				3: [{
					'Set': 'BRM',
					'Name': 'Flamewaker'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Kirin Tor Mage'
				}, {
					'Set': 'GVG',
					'Name': 'Soot Spewer'
				}],
				4: [{
					'Set': 'LOE',
					'Name': 'Animated Armor'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ethereal Arcanist'
				}, {
					'Set': 'GVG',
					'Name': 'Goblin Blastmage'
				}]
			},
			'Common': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Mana Wyrm'
				}],
				2: [{
					'Set': 'GVG',
					'Name': 'Snowchugger'
				}, {
					'Set': 'EXPERT1',
					'Name': "Sorcerer's Apprentice"
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Spellslinger'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Dalaran Aspirant'
				}],
				5: [{
					'Set': 'LOE',
					'Name': 'Ethereal Conjurer'
				}]
			},
			'Free': {
				4: [{
					'Set': 'CORE',
					'Name': 'Water Elemental'
				}]
			}
		},
		'Spell': {
			'Epic': {
				1: [{
					'Set': 'TGT',
					'Name': 'Arcane Blast'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Spellbender'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ice Block'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Echo of Medivh'
				}],
				10: [{
					'Set': 'EXPERT1',
					'Name': 'Pyroblast'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'GVG',
					'Name': 'Unstable Portal'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Polymorph: Boar'
				}, {
					'Set': 'TGT',
					'Name': 'Effigy'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Counterspell'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Vaporize'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Blizzard'
				}]
			},
			'Common': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Ice Lance'
				}],
				2: [{
					'Set': 'GVG',
					'Name': 'Flamecannon'
				}],
				3: [{
					'Set': 'NAXX',
					'Name': 'Duplicate'
				}, {
					'Set': 'LOE',
					'Name': 'Forgotten Torch'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Ice Barrier'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Mirror Entity'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Cone of Cold'
				}],
				5: [{
					'Set': 'BRM',
					'Name': "Dragon's Breath"
				}, {
					'Set': 'TGT',
					'Name': 'Flame Lance'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Arcane Missiles'
				}, {
					'Set': 'CORE',
					'Name': 'Mirror Image'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Frostbolt'
				}, {
					'Set': 'CORE',
					'Name': 'Arcane Explosion'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Frost Nova'
				}, {
					'Set': 'CORE',
					'Name': 'Arcane Intellect'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Fireball'
				}, {
					'Set': 'CORE',
					'Name': 'Polymorph'
				}],
				7: [{
					'Set': 'CORE',
					'Name': 'Flamestrike'
				}]
			}
		}
	},
	'Shaman': {
		'Minion': {
			'Legendary': {
				6: [{
					'Set': 'TGT',
					'Name': 'The Mistcaller'
				}],
				7: [{
					'Set': 'GVG',
					'Name': 'Neptulon'
				}],
				8: [{
					'Set': 'EXPERT1',
					'Name': "Al'Akir the Windlord"
				}]
			},
			'Epic': {
				4: [{
					'Set': 'GVG',
					'Name': 'Siltfin Spiritwalker'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Earth Elemental'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'GVG',
					'Name': 'Vitality Totem'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Mana Tide Totem'
				}],
				4: [{
					'Set': 'GVG',
					'Name': 'Dunemaul Shaman'
				}, {
					'Set': 'TGT',
					'Name': 'Draenei Totemcarver'
				}],
				5: [{
					'Set': 'TGT',
					'Name': 'Thunder Bluff Valiant'
				}]
			},
			'Common': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Dust Devil'
				}, {
					'Set': 'LOE',
					'Name': 'Tunnel Trogg'
				}],
				2: [{
					'Set': 'GVG',
					'Name': 'Whirling Zap-o-matic'
				}, {
					'Set': 'TGT',
					'Name': 'Totem Golem'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Unbound Elemental'
				}, {
					'Set': 'TGT',
					'Name': 'Tuskarr Totemic'
				}],
				4: [{
					'Set': 'BRM',
					'Name': 'Fireguard Destroyer'
				}, {
					'Set': 'LOE',
					'Name': 'Rumbling Elemental'
				}]
			},
			'Free': {
				2: [{
					'Set': 'CORE',
					'Name': 'Flametongue Totem'
				}],
				4: [{
					'Set': 'CORE',
					'Name': 'Windspeaker'
				}],
				6: [{
					'Set': 'CORE',
					'Name': 'Fire Elemental'
				}]
			}
		},
		'Weapon': {
			'Epic': {
				4: [{
					'Set': 'TGT',
					'Name': 'Charged Hammer'
				}],
				5: [{
					'Set': 'EXPERT1',
					'Name': 'Doomhammer'
				}]
			},
			'Rare': {
				3: [{
					'Set': 'GVG',
					'Name': 'Powermace'
				}]
			},
			'Common': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Stormforged Axe'
				}]
			}
		},
		'Spell': {
			'Epic': {
				3: [{
					'Set': 'TGT',
					'Name': 'Elemental Destruction'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Far Sight'
				}],
				4: [{
					'Set': 'GVG',
					'Name': "Ancestor's Call"
				}]
			},
			'Rare': {
				2: [{
					'Set': 'EXPERT1',
					'Name': 'Ancestral Spirit'
				}, {
					'Set': 'BRM',
					'Name': 'Lava Shock'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Lightning Storm'
				}, {
					'Set': 'TGT',
					'Name': 'Healing Wave'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Feral Spirit'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Lava Burst'
				}],
				7: [{
					'Set': 'LOE',
					'Name': 'Everyfin is Awesome'
				}]
			},
			'Common': {
				1: [{
					'Set': 'EXPERT1',
					'Name': 'Lightning Bolt'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Forked Lightning'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Earth Shock'
				}],
				2: [{
					'Set': 'TGT',
					'Name': 'Ancestral Knowledge'
				}, {
					'Set': 'NAXX',
					'Name': 'Reincarnate'
				}, {
					'Set': 'GVG',
					'Name': 'Crackle'
				}]
			},
			'Free': {
				0: [{
					'Set': 'CORE',
					'Name': 'Totemic Might'
				}, {
					'Set': 'CORE',
					'Name': 'Ancestral Healing'
				}],
				1: [{
					'Set': 'CORE',
					'Name': 'Rockbiter Weapon'
				}, {
					'Set': 'CORE',
					'Name': 'Frost Shock'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Windfury'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Hex'
				}],
				5: [{
					'Set': 'CORE',
					'Name': 'Bloodlust'
				}]
			}
		}
	},
	'Priest': {
		'Minion': {
			'Legendary': {
				5: [{
					'Set': 'GVG',
					'Name': "Vol'jin"
				}],
				7: [{
					'Set': 'TGT',
					'Name': 'Confessor Paletress'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Prophet Velen'
				}]
			},
			'Epic': {
				1: [{
					'Set': 'GVG',
					'Name': 'Shadowbomber'
				}],
				3: [{
					'Set': 'TGT',
					'Name': 'Shadowfiend'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Cabal Shadow Priest'
				}]
			},
			'Rare': {
				2: [{
					'Set': 'TGT',
					'Name': 'Wyrmrest Agent'
				}, {
					'Set': 'GVG',
					'Name': 'Shadowboxer'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Lightwell'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Spawn of Shadows'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Auchenai Soulpriest'
				}],
				5: [{
					'Set': 'GVG',
					'Name': 'Upgraded Repair Bot'
				}]
			},
			'Common': {
				1: [{
					'Set': 'BRM',
					'Name': 'Twilight Whelp'
				}],
				2: [{
					'Set': 'GVG',
					'Name': 'Shrinkmeister'
				}, {
					'Set': 'LOE',
					'Name': 'Museum Curator'
				}],
				3: [{
					'Set': 'NAXX',
					'Name': 'Dark Cultist'
				}],
				4: [{
					'Set': 'TGT',
					'Name': 'Holy Champion'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Lightspawn'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Temple Enforcer'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Northshire Cleric'
				}]
			}
		},
		'Spell': {
			'Epic': {
				2: [{
					'Set': 'TGT',
					'Name': 'Confuse'
				}],
				3: [{
					'Set': 'EXPERT1',
					'Name': 'Shadowform'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Mindgames'
				}],
				6: [{
					'Set': 'GVG',
					'Name': 'Lightbomb'
				}]
			},
			'Rare': {
				1: [{
					'Set': 'GVG',
					'Name': 'Light of the Naaru'
				}],
				2: [{
					'Set': 'BRM',
					'Name': 'Resurrect'
				}, {
					'Set': 'TGT',
					'Name': 'Convert'
				}],
				4: [{
					'Set': 'EXPERT1',
					'Name': 'Mass Dispel'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Shadow Madness'
				}],
				5: [{
					'Set': 'LOE',
					'Name': 'Excavated Evil'
				}],
				6: [{
					'Set': 'EXPERT1',
					'Name': 'Holy Fire'
				}]
			},
			'Common': {
				0: [{
					'Set': 'EXPERT1',
					'Name': 'Silence'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Circle of Healing'
				}],
				1: [{
					'Set': 'TGT',
					'Name': 'Power Word: Glory'
				}, {
					'Set': 'TGT',
					'Name': 'Flash Heal'
				}, {
					'Set': 'EXPERT1',
					'Name': 'Inner Fire'
				}],
				3: [{
					'Set': 'GVG',
					'Name': "Velen's Chosen"
				}, {
					'Set': 'EXPERT1',
					'Name': 'Thoughtsteal'
				}],
				6: [{
					'Set': 'LOE',
					'Name': 'Entomb'
				}]
			},
			'Free': {
				1: [{
					'Set': 'CORE',
					'Name': 'Mind Vision'
				}, {
					'Set': 'CORE',
					'Name': 'Power Word: Shield'
				}, {
					'Set': 'CORE',
					'Name': 'Holy Smite'
				}],
				2: [{
					'Set': 'CORE',
					'Name': 'Shadow Word: Pain'
				}, {
					'Set': 'CORE',
					'Name': 'Mind Blast'
				}, {
					'Set': 'CORE',
					'Name': 'Divine Spirit'
				}],
				3: [{
					'Set': 'CORE',
					'Name': 'Shadow Word: Death'
				}],
				5: [{
					'Set': 'CORE',
					'Name': 'Holy Nova'
				}],
				10: [{
					'Set': 'CORE',
					'Name': 'Mind Control'
				}]
			}
		}
	}
}







cardDictAttributes = {
	'Warlock': {
		'Minion': {
			'Legendary': {
				6: {
					4: {
						4: ['Wilfred Fizzlebang']
					}
				},
				9: {
					3: {
						15: ['Lord Jaraxxus']
					},
					9: {
						7: ["Mal'Ganis"]
					}
				}
			},
			'Epic': {
				4: {
					1: {
						1: ['Dreadsteed']
					},
					5: {
						6: ['Pit Lord']
					}
				},
				6: {
					9: {
						9: ['Anima Golem']
					}
				}
			},
			'Rare': {
				1: {
					1: {
						1: ['Reliquary Seeker']
					}
				},
				2: {
					1: {
						4: ['Mistress of Pain']
					},
					3: {
						2: ['Tiny Knight of Evil']
					}
				},
				3: {
					3: {
						3: ['Void Terror'],
						5: ['Felguard']
					}
				},
				4: {
					3: {
						5: ['Fel Cannon']
					}
				},
				5: {
					5: {
						7: ['Doomguard']
					}
				},
				6: {
					5: {
						4: ['Void Crusher']
					}
				}
			},
			'Common': {
				1: {
					0: {
						1: ['Blood Imp']
					},
					3: {
						2: ['Flame Imp']
					}
				},
				2: {
					2: {
						2: ['Dark Peddler']
					},
					4: {
						3: ['Wrathguard']
					}
				},
				3: {
					2: {
						4: ['Imp Gang Boss']
					}
				},
				4: {
					0: {
						4: ['Summoning Portal']
					},
					3: {
						4: ['Voidcaller']
					}
				},
				5: {
					4: {
						4: ['Floating Watcher']
					}
				},
				7: {
					6: {
						8: ['Fearsome Doomguard']
					}
				}
			},
			'Free': {
				1: {
					1: {
						3: ['Voidwalker']
					}
				},
				2: {
					4: {
						3: ['Succubus']
					}
				},
				6: {
					6: {
						6: ['Dread Infernal']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				5: ['Bane of Doom', 'Demonheart'],
				6: ['Dark Bargain'],
				8: ['Twisting Nether']
			},
			'Rare': {
				3: ['Demonwrath'],
				4: ['Imp-losion', 'Shadowflame', 'Fist of Jaraxxus'],
				6: ['Siphon Soul']
			},
			'Common': {
				1: ['Power Overwhelming'],
				2: ['Demonfire', 'Demonfuse', 'Curse of Rafaam', 'Darkbomb'],
				3: ['Sense Demons']
			},
			'Free': {
				0: ['Sacrificial Pact'],
				1: ['Mortal Coil', 'Soulfire', 'Corruption'],
				3: ['Drain Life', 'Shadow Bolt'],
				4: ['Hellfire']
			}
		}
	},
	'Warrior': {
		'Minion': {
			'Legendary': {
				6: {
					6: {
						5: ['Iron Juggernaut']
					}
				},
				8: {
					4: {
						9: ['Grommash Hellscream']
					}
				},
				10: {
					7: {
						7: ['Varian Wrynn']
					}
				}
			},
			'Epic': {
				4: {
					5: {
						3: ['Magnataur Alpha']
					}
				},
				6: {
					6: {
						7: ['Sea Reaver']
					}
				}
			},
			'Rare': {
				2: {
					1: {
						4: ['Armorsmith']
					},
					2: {
						3: ["Alexstrasza's Champion"]
					},
					3: {
						2: ['Sparring Partner']
					}
				},
				3: {
					2: {
						4: ['Frothing Berserker']
					}
				},
				4: {
					2: {
						5: ['Screwjank Clunker']
					}
				},
				5: {
					5: {
						5: ['Siege Engine']
					}
				},
				6: {
					5: {
						5: ['Shieldmaiden']
					}
				}
			},
			'Common': {
				1: {
					1: {
						3: ['Warbot']
					}
				},
				2: {
					2: {
						2: ['Cruel Taskmaster']
					}
				},
				3: {
					3: {
						3: ['Orgrimmar Aspirant'],
						4: ['Fierce Monkey']
					}
				},
				4: {
					2: {
						5: ['Axe Flinger']
					},
					3: {
						3: ['Arathi Weaponsmith']
					}
				},
				7: {
					7: {
						7: ['Obsidian Destroyer']
					}
				}
			},
			'Free': {
				3: {
					2: {
						3: ['Warsong Commander']
					}
				},
				4: {
					4: {
						3: ["Kor'kron Elite"]
					}
				}
			}
		},
		'Weapon': {
			'Epic': {
				7: {
					7: {
						1: ['Gorehowl']
					}
				}
			},
			'Rare': {
				1: {
					2: {
						3: ['Cursed Blade']
					}
				},
				3: {
					3: {
						2: ["King's Defender"]
					}
				}
			},
			'Common': {
				3: {
					4: {
						2: ['Ogre Warmaul']
					}
				},
				4: {
					4: {
						2: ["Death's Bite"]
					}
				}
			},
			'Free': {
				2: {
					3: {
						2: ['Fiery War Axe']
					}
				},
				5: {
					5: {
						2: ['Arcanite Reaper']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				1: ['Shield Slam'],
				3: ['Bouncing Blade'],
				5: ['Brawl'],
				7: ['Crush']
			},
			'Rare': {
				1: ['Upgrade!'],
				2: ['Commanding Shout', 'Revenge'],
				4: ['Mortal Strike']
			},
			'Common': {
				0: ['Inner Rage'],
				2: ['Rampage', 'Slam', 'Bolster', 'Battle Rage'],
				3: ['Bash']
			},
			'Free': {
				1: ['Execute', 'Whirlwind'],
				2: ['Heroic Strike', 'Cleave'],
				3: ['Shield Block', 'Charge']
			}
		}
	},
	'Paladin': {
		'Minion': {
			'Legendary': {
				5: {
					1: {
						7: ['Bolvar Fordragon']
					}
				},
				7: {
					3: {
						7: ['Eadric the Pure']
					}
				},
				8: {
					6: {
						6: ['Tirion Fordring']
					}
				}
			},
			'Epic': {
				5: {
					2: {
						5: ['Quartermaster']
					}
				},
				6: {
					6: {
						6: ['Mysterious Challenger']
					}
				}
			},
			'Rare': {
				3: {
					3: {
						3: ['Aldor Peacekeeper']
					},
					4: {
						3: ['Scarlet Purifier']
					}
				},
				5: {
					5: {
						5: ['Dragon Consort', 'Tuskarr Jouster']
					},
					6: {
						3: ['Cobalt Guardian']
					}
				}
			},
			'Common': {
				2: {
					2: {
						2: ['Shielded Minibot', 'Argent Protector']
					}
				},
				3: {
					2: {
						4: ['Warhorse Trainer']
					}
				},
				4: {
					3: {
						4: ['Keeper of Uldaman', 'Murloc Knight']
					}
				}
			},
			'Free': {
				7: {
					5: {
						6: ['Guardian of Kings']
					}
				}
			}
		},
		'Weapon': {
			'Epic': {
				3: {
					1: {
						5: ['Sword of Justice']
					},
					2: {
						3: ['Coghammer']
					}
				}
			},
			'Rare': {
				2: {
					2: {
						2: ['Argent Lance']
					}
				}
			},
			'Free': {
				1: {
					1: {
						4: ["Light's Justice"]
					}
				},
				4: {
					4: {
						2: ['Truesilver Champion']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				6: ['Enter the Coliseum', 'Avenging Wrath'],
				8: ['Lay on Hands']
			},
			'Rare': {
				1: ['Competitive Spirit'],
				2: ['Equality'],
				3: ['Divine Favor', 'Muster for Battle'],
				5: ['Holy Wrath', 'Blessed Champion'],
				10: ['Anyfin Can Happen']
			},
			'Common': {
				1: ['Avenge', 'Redemption', 'Eye for an Eye', 'Blessing of Wisdom', 'Noble Sacrifice', 'Repentance', 'Sacred Trial'],
				2: ['Seal of Light'],
				3: ['Seal of Champions'],
				5: ['Solemn Vigil']
			},
			'Free': {
				1: ['Blessing of Might', 'Humility', 'Hand of Protection'],
				2: ['Holy Light'],
				4: ['Hammer of Wrath', 'Consecration', 'Blessing of Kings']
			}
		}
	},
	'Hunter': {
		'Minion': {
			'Legendary': {
				3: {
					4: {
						2: ['Dreadscale']
					}
				},
				7: {
					4: {
						2: ['Acidmaw']
					},
					6: {
						9: ["Gahz'rilla"]
					}
				},
				9: {
					8: {
						8: ['King Krush']
					}
				}
			},
			'Epic': {
				2: {
					2: {
						3: ['Steamwheedle Sniper']
					}
				},
				3: {
					4: {
						2: ['Stablemaster']
					}
				}
			},
			'Rare': {
				3: {
					3: {
						3: ['Metaltooth Leaper']
					}
				},
				4: {
					4: {
						4: ['Core Rager']
					}
				},
				5: {
					2: {
						6: ['King of Beasts']
					},
					3: {
						3: ['Ram Wrangler']
					}
				},
				6: {
					6: {
						5: ['Savannah Highmane']
					}
				}
			},
			'Common': {
				1: {
					1: {
						1: ['Webspinner']
					},
					2: {
						1: ['Brave Archer']
					}
				},
				2: {
					2: {
						2: ['Scavenging Hyena']
					},
					3: {
						2: ["King's Elekk"]
					}
				},
				3: {
					2: {
						4: ['Desert Camel']
					}
				}
			},
			'Free': {
				1: {
					1: {
						1: ['Timber Wolf']
					}
				},
				4: {
					4: {
						3: ['Houndmaster']
					}
				},
				5: {
					2: {
						5: ['Tundra Rhino']
					},
					3: {
						2: ['Starving Buzzard']
					}
				}
			}
		},
		'Weapon': {
			'Epic': {
				7: {
					5: {
						2: ["Gladiator's Longbow"]
					}
				}
			},
			'Rare': {
				3: {
					3: {
						2: ['Eaglehorn Bow']
					}
				}
			},
			'Common': {
				2: {
					2: {
						2: ['Glaivezooka']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				1: ['Bestial Wrath'],
				2: ['Snake Trap', 'Feign Death', 'Lock and Load']
			},
			'Rare': {
				2: ['Misdirection', 'Flare', "Explorer's Hat", 'Call Pet'],
				3: ['Powershot'],
				5: ['Explosive Shot'],
				6: ['Ball of Spiders']
			},
			'Common': {
				2: ['Snipe', 'Quick Shot', 'Dart Trap', 'Explosive Trap', 'Bear Trap', 'Freezing Trap'],
				3: ['Deadly Shot', 'Unleash the Hounds'],
				5: ['Cobra Shot']
			},
			'Free': {
				0: ["Hunter's Mark"],
				1: ['Tracking', 'Arcane Shot'],
				3: ['Animal Companion', 'Kill Command'],
				4: ['Multi-Shot']
			}
		}
	},
	'Druid': {
		'Minion': {
			'Legendary': {
				7: {
					9: {
						7: ['Malorne']
					}
				},
				9: {
					5: {
						5: ['Aviana'],
						8: ['Cenarius']
					}
				}
			},
			'Epic': {
				7: {
					5: {
						5: ['Ancient of War', 'Ancient of Lore']
					}
				}
			},
			'Rare': {
				2: {
					2: {
						3: ['Darnassus Aspirant']
					}
				},
				3: {
					2: {
						4: ['Grove Tender']
					}
				},
				4: {
					2: {
						4: ['Keeper of the Grove']
					},
					4: {
						4: ['Jungle Moonkin']
					},
					5: {
						4: ['Savage Combatant']
					}
				},
				6: {
					7: {
						6: ['Mech-Bear-Cat']
					}
				},
				7: {
					6: {
						6: ['Knight of the Wild']
					}
				},
				9: {
					7: {
						8: ['Volcanic Lumberer']
					}
				}
			},
			'Common': {
				2: {
					2: {
						1: ['Druid of the Saber'],
						2: ['Anodized Robo Cub']
					}
				},
				3: {
					2: {
						2: ['Druid of the Flame']
					},
					3: {
						2: ['Mounted Raptor']
					}
				},
				4: {
					4: {
						4: ['Wildwalker']
					}
				},
				5: {
					4: {
						4: ['Druid of the Claw', 'Druid of the Fang']
					}
				}
			},
			'Free': {
				8: {
					8: {
						8: ['Ironbark Protector']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				3: ['Mulch'],
				4: ['Astral Communion'],
				6: ['Dark Wispers', 'Force of Nature'],
				9: ['Tree of Life']
			},
			'Rare': {
				1: ['Savagery'],
				4: ['Bite'],
				5: ['Starfall', 'Nourish'],
				6: ['Recycle']
			},
			'Common': {
				1: ['Living Roots', 'Naturalize', 'Raven Idol'],
				2: ['Power of the Wild', 'Wrath'],
				3: ['Mark of Nature'],
				4: ['Soul of the Forest', 'Poison Seeds']
			},
			'Free': {
				0: ['Innervate', 'Moonfire'],
				1: ['Claw'],
				2: ['Mark of the Wild', 'Wild Growth'],
				3: ['Savage Roar', 'Healing Touch'],
				4: ['Swipe'],
				6: ['Starfire']
			}
		}
	},
	'Neutral': {
		'Minion': {
			'Legendary': {
				1: {
					1: {
						3: ['Sir Finley Mrrgglton']
					}
				},
				2: {
					0: {
						4: ['Nat Pagle', 'Lorewalker Cho']
					},
					1: {
						1: ['Bloodmage Thalnos']
					},
					4: {
						4: ['Millhouse Manastorm']
					}
				},
				3: {
					2: {
						4: ['Brann Bronzebeard']
					},
					3: {
						3: ['Tinkmaster Overspark'],
						4: ['Eydis Darkbane', 'Fjola Lightbane']
					},
					5: {
						5: ['King Mukla']
					}
				},
				4: {
					1: {
						7: ['Baron Rivendare']
					},
					2: {
						4: ['Old Murk-Eye']
					},
					3: {
						5: ['Elise Starseeker']
					},
					4: {
						4: ['Gormok the Impaler']
					}
				},
				5: {
					3: {
						4: ['Blingtron 3000']
					},
					4: {
						5: ['Nexus-Champion Saraad', "Mimiron's Head"],
						7: ['Feugen']
					},
					5: {
						4: ['Harrison Jones', 'Captain Greenskin'],
						5: ['Loatheb', 'Elite Tauren Chieftain']
					},
					6: {
						2: ['Leeroy Jenkins'],
						3: ['Hemet Nesingwary']
					},
					7: {
						4: ['Stalagg']
					}
				},
				6: {
					2: {
						8: ['Maexxna']
					},
					3: {
						6: ['Gazlowe'],
						9: ['Bolf Ramshield']
					},
					4: {
						4: ['Hogger'],
						5: ['Cairne Bloodhoof', 'The Black Knight'],
						6: ['Reno Jackson']
					},
					5: {
						5: ['Emperor Thaurissan', 'Sylvanas Windrunner'],
						7: ['Toshley']
					},
					6: {
						3: ['Justicar Trueheart'],
						6: ['Gelbin Mekkatorque']
					},
					7: {
						4: ['The Skeleton Knight'],
						5: ['Illidan Stormrage'],
						6: ['Mogor the Ogre']
					},
					9: {
						7: ['The Beast']
					}
				},
				7: {
					4: {
						6: ["Skycap'n Kragg"]
					},
					6: {
						6: ['Troggzor the Earthinator', 'Chillmaw']
					},
					7: {
						5: ['Baron Geddon'],
						7: ['Dr. Boom']
					},
					8: {
						4: ['Rend Blackhand']
					}
				},
				8: {
					5: {
						7: ["Sneed's Old Shredder"]
					},
					6: {
						8: ['Chromaggus', "Kel'Thuzad"],
						9: ['Foe Reaper 4000']
					},
					7: {
						7: ['Gruul']
					},
					8: {
						8: ['Ragnaros the Firelord']
					}
				},
				9: {
					4: {
						12: ['Ysera', 'Malygos']
					},
					7: {
						8: ['Arch-Thief Rafaam']
					},
					8: {
						8: ['Nefarian', 'Nozdormu', 'Alexstrasza', 'Onyxia']
					},
					9: {
						7: ['Majordomo Executus', 'Mekgineer Thermaplugg']
					},
					10: {
						10: ['Icehowl']
					}
				},
				10: {
					12: {
						12: ['Deathwing']
					}
				}
			},
			'Epic': {
				1: {
					1: {
						2: ['Hungry Crab']
					}
				},
				2: {
					0: {
						7: ['Doomsayer']
					},
					1: {
						1: ["Captain's Parrot"],
						2: ['Echoing Ooze']
					},
					2: {
						3: ['Garrison Commander']
					},
					3: {
						2: ['Recombobulator']
					}
				},
				3: {
					2: {
						2: ['Shade of Naxxramas'],
						3: ['Hobgoblin']
					},
					3: {
						3: ['Murloc Warleader', 'Southsea Captain', 'Blood Knight']
					},
					4: {
						2: ['Big Game Hunter', 'Master of Ceremonies']
					}
				},
				4: {
					2: {
						6: ['Twilight Guardian']
					},
					3: {
						2: ['Enhance-o Mechano']
					},
					4: {
						1: ['Mini-Mage'],
						4: ['Crowd Favorite']
					}
				},
				5: {
					1: {
						5: ['Junkbot']
					},
					3: {
						3: ['Faceless Manipulator']
					},
					4: {
						6: ['Djinni of Zephyrs']
					},
					5: {
						4: ['Recruiter'],
						5: ['Naga Sea Witch']
					},
					8: {
						8: ['Fel Reaver']
					}
				},
				6: {
					3: {
						5: ['Kodorider']
					},
					5: {
						5: ['Grand Crusader']
					},
					6: {
						4: ['Piloted Sky Golem'],
						5: ['Sideshow Spelleater']
					}
				},
				10: {
					8: {
						8: ['Sea Giant', 'Frost Giant']
					}
				},
				12: {
					8: {
						8: ['Mountain Giant', 'Clockwork Giant']
					}
				},
				20: {
					8: {
						8: ['Molten Giant']
					}
				}
			},
			'Rare': {
				0: {
					0: {
						2: ['Target Dummy']
					}
				},
				1: {
					0: {
						2: ['Dragon Egg']
					},
					1: {
						1: ['Angry Chicken'],
						2: ['Murloc Tidecaller', 'Secretkeeper', 'Bloodsail Corsair', 'Lightwarden']
					},
					2: {
						1: ['Young Priestess'],
						4: ['Injured Kvaldir']
					}
				},
				2: {
					0: {
						2: ['Nerubian Egg']
					},
					1: {
						3: ['Master Swordsmith', 'Mana Addict']
					},
					2: {
						2: ['Pint-Sized Summoner', 'Crazed Alchemist', 'Mana Wraith'],
						3: ['Sunfury Protector'],
						4: ['Argent Watchman']
					},
					3: {
						2: ['Wild Pyromancer', 'Knife Juggler']
					},
					4: {
						5: ['Ancient Watcher']
					}
				},
				3: {
					0: {
						3: ['Alarm-o-Bot']
					},
					1: {
						4: ['Demolisher'],
						5: ['Imp Master']
					},
					2: {
						2: ['Questing Adventurer', 'Fencing Coach', 'Coldlight Oracle'],
						3: ['Emperor Cobra', "Lil' Exorcist", 'Coldlight Seer'],
						4: ['Illuminator', 'Goblin Sapper'],
						5: ['Coliseum Manager'],
						8: ['Deathlord']
					},
					3: {
						2: ['Gnomish Experimenter'],
						3: ['Mind Control Tech']
					},
					4: {
						2: ['Arcane Golem'],
						3: ["Light's Champion", 'Saboteur'],
						7: ['Injured Blademaster']
					}
				},
				4: {
					1: {
						4: ['Jeeves']
					},
					2: {
						3: ['Defender of Argus'],
						5: ['Ancient Mage', 'Arcane Nullifier X-21']
					},
					3: {
						5: ['Wailing Soul', 'Violet Teacher']
					},
					4: {
						1: ['Twilight Drake'],
						3: ['Kezan Mystic']
					},
					5: {
						3: ['Armored Warhorse']
					},
					7: {
						4: ['Ancient Shade'],
						7: ['Eerie Statue']
					}
				},
				5: {
					0: {
						6: ['Summoning Stone']
					},
					3: {
						3: ['Grim Patron', 'Bomb Lobber'],
						5: ['Stampeding Kodo', 'Sludge Belcher']
					},
					4: {
						4: ['Azure Drake', 'Abomination']
					},
					5: {
						4: ['Madder Bomber']
					}
				},
				6: {
					2: {
						6: ['Wobbling Runts']
					},
					4: {
						2: ['Argent Commander'],
						4: ['Gadgetzan Auctioneer'],
						5: ['Sunwalker']
					},
					5: {
						6: ['Master Jouster']
					},
					8: {
						5: ["Mogor's Champion"]
					}
				},
				7: {
					7: {
						5: ['Ravenholdt Assassin']
					}
				}
			},
			'Common': {
				0: {
					1: {
						1: ['Murloc Tinyfin', 'Wisp']
					}
				},
				1: {
					0: {
						4: ['Shieldbearer']
					},
					1: {
						1: ['Young Dragonhawk', 'Argent Squire'],
						2: ['Undertaker', 'Gadgetzan Jouster', 'Lowly Squire', 'Cogmaster']
					},
					2: {
						1: ['Tournament Attendee', 'Clockwork Gnome', 'Southsea Deckhand', 'Abusive Sergeant', 'Leper Gnome', 'Worgen Infiltrator'],
						3: ['Zombie Chow']
					}
				},
				2: {
					1: {
						1: ['Explosive Sheep', 'Jeweled Scarab'],
						2: ['Micro Machine', 'Haunted Creeper', 'Lance Carrier', 'Annoy-o-Tron'],
						3: ['Unstable Ghoul'],
						4: ["Nerub'ar Weblord"]
					},
					2: {
						1: ['Loot Hoarder', 'Ironbeak Owl'],
						2: ['Dire Wolf Alpha', 'Mad Scientist'],
						3: ['Stonesplinter Trogg', 'Mechwarper', 'Flame Juggler', 'Amani Berserker', 'Bloodsail Raider', "Ship's Cannon", 'Gilblin Stalker']
					},
					3: {
						2: ['Mad Bomber', 'Youthful Brewmaster', 'Faerie Dragon', 'Huge Toad', 'Boneguard Lieutenant', 'Puddlestomper']
					}
				},
				3: {
					1: {
						3: ['Acolyte of Pain'],
						4: ['Gnomeregan Infantry', 'Flying Machine', 'Stoneskin Gargoyle']
					},
					2: {
						1: ['Argent Horserider'],
						2: ['Silent Knight'],
						3: ['Thrallmar Farseer', 'Harvest Golem', 'Flesheating Ghoul', 'Tauren Warrior'],
						4: ['Blackwing Technician']
					},
					3: {
						1: ['Scarlet Crusader'],
						3: ['Raging Worgen', 'Tinkertown Technician', 'Dragonhawk Rider', 'Earthen Ring Farseer', 'Silver Hand Regent'],
						4: ['Spider Tank']
					},
					4: {
						2: ['Jungle Panther'],
						4: ['Dancing Swords', 'Ogre Brute']
					},
					5: {
						2: ['Ice Rager']
					}
				},
				4: {
					1: {
						7: ["Mogu'shan Warden"],
						8: ['Tournament Medic']
					},
					2: {
						6: ['Maiden of the Lake', 'Frigid Snobold']
					},
					3: {
						3: ['Dread Corsair', 'Tomb Spider', 'Silvermoon Guardian'],
						4: ['Gorillabot A-3'],
						5: ['Refreshment Vendor', 'Burly Rockjaw Trogg', 'Dragonkin Sorcerer']
					},
					4: {
						2: ['Cult Master'],
						3: ['Piloted Shredder', 'Spellbreaker'],
						4: ['Dark Iron Dwarf'],
						5: ['Mechanical Yeti']
					},
					5: {
						4: ['Ancient Brewmaster', 'Lost Tallstrider', 'Evil Heckler'],
						6: ['Hungry Dragon']
					}
				},
				5: {
					3: {
						3: ['Antique Healbot'],
						6: ['Fen Creeper']
					},
					4: {
						3: ["Mukla's Champion"],
						4: ['Anubisath Sentinel', 'Kvaldir Raider', 'Silver Hand Knight'],
						6: ['Spiteful Smith', 'Spectral Knight']
					},
					5: {
						4: ['Blackwing Corruptor'],
						5: ['Clockwork Knight', 'Stranglethorn Tiger'],
						6: ['Pit Fighter']
					},
					7: {
						4: ['Salty Dog'],
						6: ['Venture Co. Mercenary']
					}
				},
				6: {
					4: {
						5: ['Windfury Harpy']
					},
					5: {
						4: ['Priestess of Elune'],
						5: ['Frost Elemental']
					},
					6: {
						4: ['Volcanic Drake'],
						6: ['Drakonid Crusher']
					}
				},
				7: {
					5: {
						9: ['Captured Jormungar']
					}
				},
				8: {
					7: {
						7: ['Force-Tank MAX']
					},
					8: {
						8: ['Fossilized Devilsaur']
					}
				},
				9: {
					9: {
						7: ['North Sea Kraken']
					}
				}
			},
			'Free': {
				1: {
					1: {
						1: ['Elven Archer', 'Grimscale Oracle', 'Stonetusk Boar'],
						2: ['Goldshire Footman']
					},
					2: {
						1: ['Voodoo Doctor', 'Murloc Raider']
					}
				},
				2: {
					1: {
						1: ['Novice Engineer']
					},
					2: {
						1: ['Bluegill Warrior', 'Murloc Tidehunter'],
						2: ['Kobold Geomancer', 'Frostwolf Grunt'],
						3: ['River Crocolisk']
					},
					3: {
						2: ['Acidic Swamp Ooze', 'Bloodfen Raptor']
					}
				},
				3: {
					1: {
						4: ['Silverback Patriarch', 'Dalaran Mage']
					},
					2: {
						2: ['Ironforge Rifleman', 'Raid Leader'],
						3: ['Razorfen Hunter']
					},
					3: {
						1: ['Wolfrider'],
						2: ['Shattered Sun Cleric'],
						3: ['Ironfur Grizzly']
					},
					5: {
						1: ['Magma Rager']
					}
				},
				4: {
					2: {
						4: ['Dragonling Mechanic', 'Gnomish Inventor'],
						5: ['Stormwind Knight'],
						7: ['Oasis Snapjaw']
					},
					3: {
						5: ["Sen'jin Shieldmasta"]
					},
					4: {
						4: ['Ogre Magi'],
						5: ['Chillwind Yeti']
					}
				},
				5: {
					2: {
						7: ['Gurubashi Berserker']
					},
					4: {
						2: ['Stormpike Commando'],
						4: ['Nightblade', 'Frostwolf Warlord'],
						5: ['Darkscale Healer']
					},
					5: {
						4: ['Booty Bay Bodyguard']
					}
				},
				6: {
					4: {
						7: ['Archmage']
					},
					5: {
						2: ['Reckless Rocketeer']
					},
					6: {
						5: ['Lord of the Arena'],
						7: ['Boulderfist Ogre']
					}
				},
				7: {
					6: {
						6: ['Stormwind Champion']
					},
					7: {
						7: ['War Golem']
					},
					9: {
						5: ['Core Hound']
					}
				}
			}
		}
	},
	'Rogue': {
		'Minion': {
			'Legendary': {
				3: {
					2: {
						2: ['Edwin VanCleef']
					}
				},
				6: {
					5: {
						8: ['Trade Prince Gallywix']
					}
				},
				9: {
					8: {
						4: ["Anub'arak"]
					}
				}
			},
			'Epic': {
				2: {
					1: {
						1: ['Patient Assassin']
					}
				},
				6: {
					5: {
						3: ['Kidnapper']
					}
				}
			},
			'Rare': {
				2: {
					2: {
						2: ['Cutpurse']
					},
					4: {
						1: ['One-eyed Cheat']
					}
				},
				3: {
					2: {
						2: ['Iron Sensei']
					},
					3: {
						3: ['SI:7 Agent'],
						4: ['Unearthed Raptor']
					},
					4: {
						3: ['Shady Dealer']
					}
				},
				4: {
					4: {
						4: ['Master of Disguise']
					}
				},
				5: {
					4: {
						3: ['Dark Iron Skulker']
					},
					6: {
						6: ['Ogre Ninja']
					}
				}
			},
			'Common': {
				1: {
					2: {
						1: ['Buccaneer', 'Pit Snake']
					}
				},
				2: {
					2: {
						2: ['Defias Ringleader']
					},
					3: {
						2: ['Goblin Auto-Barber', 'Undercity Valiant']
					}
				},
				4: {
					5: {
						4: ['Tomb Pillager'],
						5: ["Anub'ar Ambusher"]
					}
				},
				5: {
					3: {
						7: ['Shado-Pan Rider']
					}
				}
			}
		},
		'Weapon': {
			'Epic': {
				3: {
					1: {
						3: ["Cogmaster's Wrench"]
					}
				},
				4: {
					1: {
						3: ['Poisoned Blade']
					}
				}
			},
			'Rare': {
				3: {
					2: {
						2: ["Perdition's Blade"]
					}
				}
			},
			'Free': {
				5: {
					3: {
						4: ["Assassin's Blade"]
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				0: ['Preparation'],
				3: ['Beneath the Grounds'],
				4: ['Sabotage']
			},
			'Rare': {
				2: ['Blade Flurry'],
				3: ['Burgle', 'Headcrack']
			},
			'Common': {
				0: ['Shadowstep'],
				1: ['Conceal', 'Cold Blood'],
				2: ['Betrayal', 'Eviscerate', 'Gang Up'],
				4: ["Tinker's Sharpsword Oil"]
			},
			'Free': {
				0: ['Backstab'],
				1: ['Sinister Strike', 'Deadly Poison'],
				2: ['Shiv', 'Sap'],
				3: ['Fan of Knives'],
				5: ['Assassinate'],
				6: ['Vanish'],
				7: ['Sprint']
			}
		}
	},
	'Mage': {
		'Minion': {
			'Legendary': {
				7: {
					5: {
						7: ['Archmage Antonidas']
					},
					7: {
						7: ['Flame Leviathan']
					}
				},
				8: {
					7: {
						7: ['Rhonin']
					}
				}
			},
			'Epic': {
				4: {
					2: {
						5: ['Wee Spellstopper']
					}
				},
				6: {
					6: {
						6: ['Coldarra Drake']
					}
				}
			},
			'Rare': {
				2: {
					3: {
						2: ['Fallen Hero']
					}
				},
				3: {
					2: {
						4: ['Flamewaker']
					},
					3: {
						3: ['Soot Spewer']
					},
					4: {
						3: ['Kirin Tor Mage']
					}
				},
				4: {
					3: {
						3: ['Ethereal Arcanist']
					},
					4: {
						4: ['Animated Armor']
					},
					5: {
						4: ['Goblin Blastmage']
					}
				}
			},
			'Common': {
				1: {
					1: {
						3: ['Mana Wyrm']
					}
				},
				2: {
					2: {
						3: ['Snowchugger']
					},
					3: {
						2: ["Sorcerer's Apprentice"]
					}
				},
				3: {
					3: {
						4: ['Spellslinger']
					}
				},
				4: {
					3: {
						5: ['Dalaran Aspirant']
					}
				},
				5: {
					6: {
						3: ['Ethereal Conjurer']
					}
				}
			},
			'Free': {
				4: {
					3: {
						6: ['Water Elemental']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				1: ['Arcane Blast'],
				3: ['Spellbender', 'Ice Block'],
				4: ['Echo of Medivh'],
				10: ['Pyroblast']
			},
			'Rare': {
				2: ['Unstable Portal'],
				3: ['Polymorph: Boar', 'Effigy', 'Counterspell', 'Vaporize'],
				6: ['Blizzard']
			},
			'Common': {
				1: ['Ice Lance'],
				2: ['Flamecannon'],
				3: ['Duplicate', 'Forgotten Torch', 'Ice Barrier', 'Mirror Entity'],
				4: ['Cone of Cold'],
				5: ["Dragon's Breath", 'Flame Lance']
			},
			'Free': {
				1: ['Arcane Missiles', 'Mirror Image'],
				2: ['Frostbolt', 'Arcane Explosion'],
				3: ['Frost Nova', 'Arcane Intellect'],
				4: ['Fireball', 'Polymorph'],
				7: ['Flamestrike']
			}
		}
	},
	'Shaman': {
		'Minion': {
			'Legendary': {
				6: {
					4: {
						4: ['The Mistcaller']
					}
				},
				7: {
					7: {
						7: ['Neptulon']
					}
				},
				8: {
					3: {
						5: ["Al'Akir the Windlord"]
					}
				}
			},
			'Epic': {
				4: {
					2: {
						5: ['Siltfin Spiritwalker']
					}
				},
				5: {
					7: {
						8: ['Earth Elemental']
					}
				}
			},
			'Rare': {
				2: {
					0: {
						3: ['Vitality Totem']
					}
				},
				3: {
					0: {
						3: ['Mana Tide Totem']
					}
				},
				4: {
					4: {
						4: ['Draenei Totemcarver']
					},
					5: {
						4: ['Dunemaul Shaman']
					}
				},
				5: {
					3: {
						6: ['Thunder Bluff Valiant']
					}
				}
			},
			'Common': {
				1: {
					1: {
						3: ['Tunnel Trogg']
					},
					3: {
						1: ['Dust Devil']
					}
				},
				2: {
					3: {
						2: ['Whirling Zap-o-matic'],
						4: ['Totem Golem']
					}
				},
				3: {
					2: {
						4: ['Unbound Elemental']
					},
					3: {
						2: ['Tuskarr Totemic']
					}
				},
				4: {
					2: {
						6: ['Rumbling Elemental']
					},
					3: {
						6: ['Fireguard Destroyer']
					}
				}
			},
			'Free': {
				2: {
					0: {
						3: ['Flametongue Totem']
					}
				},
				4: {
					3: {
						3: ['Windspeaker']
					}
				},
				6: {
					6: {
						5: ['Fire Elemental']
					}
				}
			}
		},
		'Weapon': {
			'Epic': {
				4: {
					2: {
						4: ['Charged Hammer']
					}
				},
				5: {
					2: {
						8: ['Doomhammer']
					}
				}
			},
			'Rare': {
				3: {
					3: {
						2: ['Powermace']
					}
				}
			},
			'Common': {
				2: {
					2: {
						3: ['Stormforged Axe']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				3: ['Elemental Destruction', 'Far Sight'],
				4: ["Ancestor's Call"]
			},
			'Rare': {
				2: ['Ancestral Spirit', 'Lava Shock'],
				3: ['Lightning Storm', 'Healing Wave', 'Feral Spirit', 'Lava Burst'],
				7: ['Everyfin is Awesome']
			},
			'Common': {
				1: ['Lightning Bolt', 'Forked Lightning', 'Earth Shock'],
				2: ['Ancestral Knowledge', 'Reincarnate', 'Crackle']
			},
			'Free': {
				0: ['Totemic Might', 'Ancestral Healing'],
				1: ['Rockbiter Weapon', 'Frost Shock'],
				2: ['Windfury'],
				3: ['Hex'],
				5: ['Bloodlust']
			}
		}
	},
	'Priest': {
		'Minion': {
			'Legendary': {
				5: {
					6: {
						2: ["Vol'jin"]
					}
				},
				7: {
					5: {
						4: ['Confessor Paletress']
					},
					7: {
						7: ['Prophet Velen']
					}
				}
			},
			'Epic': {
				1: {
					2: {
						1: ['Shadowbomber']
					}
				},
				3: {
					3: {
						3: ['Shadowfiend']
					}
				},
				6: {
					4: {
						5: ['Cabal Shadow Priest']
					}
				}
			},
			'Rare': {
				2: {
					0: {
						5: ['Lightwell']
					},
					1: {
						4: ['Wyrmrest Agent']
					},
					2: {
						3: ['Shadowboxer']
					}
				},
				4: {
					3: {
						5: ['Auchenai Soulpriest']
					},
					5: {
						4: ['Spawn of Shadows']
					}
				},
				5: {
					5: {
						5: ['Upgraded Repair Bot']
					}
				}
			},
			'Common': {
				1: {
					2: {
						1: ['Twilight Whelp']
					}
				},
				2: {
					1: {
						2: ['Museum Curator']
					},
					3: {
						2: ['Shrinkmeister']
					}
				},
				3: {
					3: {
						4: ['Dark Cultist']
					}
				},
				4: {
					0: {
						5: ['Lightspawn']
					},
					3: {
						5: ['Holy Champion']
					}
				},
				6: {
					6: {
						6: ['Temple Enforcer']
					}
				}
			},
			'Free': {
				1: {
					1: {
						3: ['Northshire Cleric']
					}
				}
			}
		},
		'Spell': {
			'Epic': {
				2: ['Confuse'],
				3: ['Shadowform'],
				4: ['Mindgames'],
				6: ['Lightbomb']
			},
			'Rare': {
				1: ['Light of the Naaru'],
				2: ['Resurrect', 'Convert'],
				4: ['Mass Dispel', 'Shadow Madness'],
				5: ['Excavated Evil'],
				6: ['Holy Fire']
			},
			'Common': {
				0: ['Silence', 'Circle of Healing'],
				1: ['Power Word: Glory', 'Flash Heal', 'Inner Fire'],
				3: ["Velen's Chosen", 'Thoughtsteal'],
				6: ['Entomb']
			},
			'Free': {
				1: ['Mind Vision', 'Power Word: Shield', 'Holy Smite'],
				2: ['Shadow Word: Pain', 'Mind Blast', 'Divine Spirit'],
				3: ['Shadow Word: Death'],
				5: ['Holy Nova'],
				10: ['Mind Control']
			}
		}
	}
}